...
public class LlistatReceptes extends Activity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        SQLiteDatabase baseDades = helper.getWritableDatabase();

        conversor = new DatabaseConversor(helper);

        conjuntrecepta = conversor.getReceptes(0);

        ArrayList<Recepta> seleccionats = new ArrayList<Recepta>();

        for (int i = 0; i < conjuntrecepta.size(); i++) {
            if (conjuntrecepta.get(i).getTipus() == 0) {
                seleccionats.add(conjuntrecepta.get(i));
            }
        }
